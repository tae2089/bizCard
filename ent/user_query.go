...
// QueryBizCards chains the current query on the "biz_cards" edge.
func (uq *UserQuery) QueryBizCards() *BizCardQuery {
	query := &BizCardQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBizCards: uq.withBizCards.Clone(),
...
// WithBizCards tells the query-builder to eager-load the nodes that are connected to
// the "biz_cards" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithBizCards(opts ...func(*BizCardQuery)) *UserQuery {
	query := &BizCardQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withBizCards = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withBizCards; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.BizCards = []*BizCard{}
		}
		query.withFKs = true
		query.Where(predicate.BizCard(func(s *sql.Selector) {
			s.Where(sql.InValues(user.BizCardsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_biz_cards
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_biz_cards" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_biz_cards" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.BizCards = append(node.Edges.BizCards, n)
		}
	}
